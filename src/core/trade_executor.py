...
class TradeExecutor:
    ...
    async def _place_new_limit_order(self, exchange: Exchange, symbol: str, side: Side, amount: float, price: float):
        # TODO
    ...
    async def _edit_limit_order(self, exchange: Exchange, order_id: str, symbol: str, side: Side, amount: float, price: float):
        client = self.bybit_client if exchange == Exchange.BYBIT else self.hyperliquid_client
        return await client.edit_order(order_id, symbol, OrderType.LIMIT, side, amount, price)

    async def place_market_order(self, exchange: Exchange, symbol: str, side: Side, amount: float):
        client = self.bybit_client if exchange == Exchange.BYBIT else self.hyperliquid_client
        return await client.place_order(symbol, OrderType.MARKET, side, amount)

    async def cancel_order(self, exchange: Exchange, order_id: Optional[str], symbol: str):
        if not order_id:
            return
        client = self.bybit_client if exchange == Exchange.BYBIT else self.hyperliquid_client
    ...
    async def place_limit_order(self, exchange: Exchange, order_id: Optional[str], symbol: str, side: Side, amount: float, price: float) -> str:
        if not order_id:
            order_id = await self._place_new_limit_order(exchange, symbol, side, amount, price)
            logger.info(f"Placed limit order: {symbol}, {side}, {amount}, {price}, {order_id}")
        else:
            order_id = await self._edit_limit_order(exchange, order_id, symbol, side, amount, price)
            logger.info(f"Edited limit order: {symbol}, {side}, {amount}, {price}, {order_id}")
        return order_id

    async def get_precisions(self, exchange: Exchange, symbol: str) -> tuple[float, float]:
        client = self.bybit_client if exchange == Exchange.BYBIT else self.hyperliquid_client
        return await client.fetch_precision(symbol)
